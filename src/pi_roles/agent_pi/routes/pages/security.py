...
def security_pages(app, config):
    """Define routes for security pages"""

    @app.route("/manage-rooms")
    def manage_rooms_page():
        """Render the manage rooms page if the user is logged in, otherwise redirect to login"""
        try:
            ...
            return render_template("manage-rooms.html", role="Agent", rooms=rooms, config=config)
        except Exception as e:
